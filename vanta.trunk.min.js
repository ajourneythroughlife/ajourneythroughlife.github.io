! function (t, e) {
    ...
    function r(t) {
        for (; t.children && t.children.length > 0;) r(t.children[0]), t.remove(t.children[0]);
    ...
    l.VantaBase = class {
        constructor(t = {}) {
            ...
            if (this.options = Object.assign({
                    ...
                    el: t
                }), Object.assign(this.options, t), this.options.THREE && (a = this.options.THREE), this.el = this.options.el, null == this.el) c('Instance needs "el" param!');
            else if (!(this.options.el instanceof HTMLElement)) {
                const t = this.el;
                if (this.el = (i = t, document.querySelector(i)), !this.el) return void c("Cannot find element", t)
            }
            var i, s;
            ...
            try {
                ...
                return c("Init error", t), this.renderer && this.renderer.domElement && this.el.removeChild(this.renderer.domElement), void(this.options.backgroundColor && (console.log("[VANTA] Falling back to backgroundColor"), this.el.style.background = (s = this.options.backgroundColor, "number" == typeof s ? "#" + ("00000" + s.toString(16)).slice(-6) : s)))
            }
        ...
        initThree() {
            a.WebGLRenderer ? (this.renderer = new a.WebGLRenderer({
                alpha: !0,
                antialias: !0
            }), this.el.appendChild(this.renderer.domElement), this.applyCanvasStyles(this.renderer.domElement), isNaN(this.options.backgroundAlpha) && (this.options.backgroundAlpha = 1), this.scene = new a.Scene) : console.warn("[VANTA] No THREE defined on window")
        }
        getCanvasElement() {
        ...
        destroy() {
            ...
            const e = this.scene;
            e && e.children && r(e), this.renderer && (this.renderer.domElement && this.el.removeChild(this.renderer.domElement), this.renderer = null, this.scene = null), l.current === this && (l.current = null)
        }
    };
    const p = l.VantaBase;
    ...
    class u extends p {
        ...
        material(t) {
            return new d.MeshLambertMaterial({
                color: t
            })
        }
        genRing(t, e, i, r, h, a, l) {
            null == r && (r = 0), null == h && (h = 1.4 * Math.PI), null == a && (a = 0), null == l && (l = 1), this.rings || (this.rings = []), e < 1 && (e = 1);
            const c = {
                    depth: .4,
                    bevelEnabled: !1,
                    steps: 1,
                ...
                p = new d.Shape;
            p.absarc(0, 0, e + i, 0, h, !1), p.lineTo(e * Math.cos(h), e * Math.sin(h)), p.absarc(0, 0, e, h, 0, !0);
            const u = new d.ExtrudeGeometry(p, c),
                m = this.material(t);
            (0 === n(0, 1) || e > 60) && (m.transparent = !0, m.opacity = Math.max(50 / e + o(-.3, .3), .1));
            const f = new d.Mesh(u, m);
            if (f.rotation.x = Math.PI / 2, f.rotation.z = r, f.position.y = a, f.speed = .001 * l, f.receiveShadow = !0, f.castShadow = !0, this.rings.push(f), this.cont.add(f), e < 20 && h < 1.3 * Math.PI && n(0, 2)) try {
                this.genRing(s(this.colors), e + o(-1, 3), i + o(-2, 0), r + h, h + o(-.5, .5), a + o(-3, 1), l)
            } catch (t) {}
            return f
        }
        onInit() {
            ...
            this.cont = new d.Group, this.cont.position.set(30, 0, 0), this.cont.rotation.x = .06667, this.cont.rotation.z = .16667, this.scene.add(this.cont);
            let r = i() ? 30 : 60;
            for (let e = 0; e < r; e++) {
                let e;
                n(0, 3) ? (e = o(2, 4) + o(1, 30) * o(1, 2) * o(1, 2) * o(1, 2), i() && (e *= .5), t = o(0, 3.5) + o(0, 3.5) - n(0, e / 4) - e / 50) : (e = o(1, 3) * o(2, 4), t = o(1, 2) * o(1, 2) * o(1.1, 1.5));
                const r = .05 * Math.pow(2, n(0, 4));
                t < r && (t = r), this.genRing(s(this.colors), e, t, o(0, 1e3), o(1, 6), o(0, 50 / (e + 1) + 5) + 5 / t / (e + .5), .25 * Math.max(-o(.5, 2), o(1, 50 - e / 2) - e / 2))
            }
            ...
            const h = new d.AmbientLight(16777215, .5);
            return this.scene.add(h), this.pointLight = new d.PointLight(16777215, .5), this.pointLight.position.set(0, 150, 200), this.scene.add(this.pointLight), this.spot = new d.SpotLight(16777215, 1), this.spot.position.set(-15, 50, 100), this.spot.penumbra = 1, this.spot.angle = .5, this.spot.decay = 1, this.spot.distance = 300, this.spot.target = this.cont, this.scene.add(this.spot)
        }
        onUpdate() {
            let t;
            null != this.helper && this.helper.update(), null != this.controls && this.controls.update();
            const e = this.camera;
            Math.abs(e.tx - e.position.x) > .01 && (t = e.tx - e.position.x, e.position.x += .02 * t), Math.abs(e.ty - e.position.y) > .01 && (t = e.ty - e.position.y, e.position.y += .02 * t), e.lookAt(new d.Vector3(0, 25, 7)), e.near = Math.max(.5 * e.position.z - 20, 1), e.updateProjectionMatrix();
